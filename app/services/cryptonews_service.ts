...
export class CryptoNewsService {
  ...
  public static async callbackBrokerMessageForNews(message: string): Promise<News> {
    ...
    const filteredNews: NewsReceived = this.filterNews(newsReceived)
    return await this.saveNews(filteredNews)
  }
   ...
   */
  private static async saveNews(news: NewsReceived): Promise<News> {
